...
| --- | --- | --- | --- |
| `downloadDirectory` | 必填、只读路径 + 目录按钮 | 绝对、存在的目录；默认安装目录下 `download` | picker 选中后保存；失败回退 |
...
路径输入不接受键盘输入、粘贴或换行，因此无 trim/空白编辑语义。dialog 返回值按平台原样保留，不在前端 trim 或拼接；Rust 将空字符串、相对路径、不存在路径和非目录判为 `SETTINGS_PATH_INVALID`。默认 `download` 目录不存在时 Rust 尝试创建；创建失败为 load error。
...
- 默认安装目录下 download 不存在：创建；无法创建则 load error。安装目录解析失败时不拼接 `~`，返回 load error。
